...
| `fork(source, new_name, namespace=..., snapshot=None)` | `SandboxProxy` | Clone root disk (kata-qemu-longhorn) |
...
`pause`, `resume`, and `fork` work on `Client` and on the `SandboxProxy` from `create`. Fork clones the root disk via Longhorn `VolumeSnapshot` and requires the **kata-qemu-longhorn** backend.
