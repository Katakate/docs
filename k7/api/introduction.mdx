...
All endpoints return standard HTTP codes. `401 Unauthorized` if the key is missing/invalid/expired. `403 Forbidden` if a **namespace-scoped** key is used against another namespace or an all-namespaces operation. Keys created with `k7 generate-api-key NAME -n <ns>` are scoped; omit `-n` for unrestricted access.
