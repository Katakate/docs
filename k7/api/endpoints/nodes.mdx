...
- This is cluster-wide topology (node names, pool sizes, per-agent error strings), so it is authorized like an all-namespaces operation: a **namespace-scoped** API key receives `403 Forbidden`; unscoped keys are unchanged.
