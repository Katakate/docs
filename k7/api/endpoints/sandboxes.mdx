...
`VolumeSnapshot` of its root PVC. On **k7d**, freezes the live VM in place
...
replicas ≥ 2). **k7d**: warm CoW fork of disk **and** memory (~2–4 s to a
Ready pod; child inherits running processes). **kfd**: returns 400. The
request blocks until the new sandbox is usable.
