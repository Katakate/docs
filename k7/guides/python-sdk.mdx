...
```bash
pip install k7-sdk==0.4.0            # sync client (requests only)
pip install "k7-sdk[async]==0.4.0"   # also installs httpx for AsyncClient
```

Run this on your **laptop / client**. A k7 node (fresh Ubuntu 24.04) does not ship `python3-venv`; `python3 -m venv …` fails until `sudo apt install -y python3-venv python3-pip`.

The async client raises `RuntimeError` at construction time if `httpx` isn't installed.

Always pass `verify_ssl` as the cluster CA path on the default HTTPS install (`./k7-ca.crt`, or `/etc/k7/tls/ca.crt` on the node). k7 **0.4.0** `k7 api status` prints a snippet that includes it; 0.3.1 omitted it and that snippet failed against the playbook-minted CA.
...
HTTP `POST /sandboxes` (and `Client.create` / `k7 create`) returns once Kubernetes objects exist — often while `ready` is still `False`. Poll `list()` before exec or curling a NodePort:
...
def wait_until_ready(client, name, namespace="default", timeout=120):
    ...
    while time.time() < deadline:
        for s in client.list(namespace=namespace):
            ready = s.get("ready")
            if s["name"] == name and s.get("status") == "Running" and ready in (True, "True"):
                return
        time.sleep(2)
...
wait_until_ready(k7, "demo-restored")  # restore returns once objects exist; boot is async
