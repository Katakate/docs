...
```text
 ...
 ════════════════════════════════════▼═══════════════▼═════════════════ host
                     ...
                     │  ┌────────────┐ ┌────────┐ ┌─────────┐ │  many VMs
                     │  │ snapshot   │ │ native │ │ clock   │ │  owns: KVM fds, guest memory,
...
Every daemon-owned VM is a `Guest` that is either `Native` (in-process rust-vmm, the default) or `Firecracker` (stock v1.16.1 under the stock jailer, `runtimeClassName: k7-fc`). Both boot the same guest images and speak to the same agent; the engine is selected per VM by `VmConfig.backend`, the `k7d.katakate.org/backend` annotation, or the RuntimeClass. [Backends](/k7d/concepts/backends) has the matrix.
