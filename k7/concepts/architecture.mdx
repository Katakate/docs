...
flowchart TB
    ...
    subgraph Node["K7 node — Ubuntu + K3s"]
        ...
        subgraph DataPlane["Data plane<br/>per node"]
            ...
            Containerd["K3s containerd<br/>+ Kata shim<br/>(kata-fc / kata-qemu)"]
        end

        subgraph Sandbox["Sandbox VM (Kata)"]
            VMM["Firecracker (+ jailer)<br/>or QEMU"]
            UserCnt["user container"]
            VMM --- UserCnt
        end

        Storage[("Storage backends<br/>LVM thin-pool · Longhorn PVC")]
    ...
    class DataPlane,Cilium,Containerd data;
...
- `kata-fc` — Firecracker microVMs (used by `kata-firecracker-devmapper`)
...
|---------|-------|---------|---------|----------|
| `kata-firecracker-devmapper` | `kfd` | Kata + Firecracker + jailer | LVM thin-pool snapshotter | Lightest footprint, fastest cold boot, ephemeral workloads |
