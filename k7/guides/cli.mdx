...
k7 install --backend k7d --k7d-artifact /root/k7d-v0.2.1-x86_64-linux.tar.gz
...
- **--k7d-version**: k7d release to download (k7d backend only). k7 **0.2.2+** playbook default is `0.2.1`. PPA `k7` 0.2.1 still defaults to k7d 0.1.0 — pass `--k7d-version 0.2.1`.
...
`VolumeSnapshot` of its root PVC (`longhorn` `VolumeSnapshotClass`). On **k7d**,
pause freezes the live VM in place (vCPUs stop, memory stays) — no snapshot.
`--snapshot` is kata-qemu-longhorn-only.
...
On **k7d**, restarts the vCPU loop — in-memory state is intact.
...
- **`k7d`** — warm CoW fork of **disk and memory** (~5 ms at the VMM, ~2–4 s end-to-end to a Ready pod). The child inherits files, tmpfs, and running processes. Lands on the source's node. Sidecar-equipped sandboxes cannot be forked on k7d.
- **`kql` / `kata-qemu-longhorn`** — disk-only clone via a crash-consistent VolumeSnapshot of `<sandbox>-root-lh`, then a cold boot (~45 s single-node; longer with Longhorn replicas ≥ 2).
- **`kfd`** — rejected.
...
- **backend** (string, optional): `kata-firecracker-devmapper` / `kfd`, `kata-qemu-longhorn` / `kql`, or `k7d`.
- **sidecar** (string, optional): sidecar type, e.g. `docker`.
...
- **-b, --backend**: `kfd` / `kql` / `k7d` (or the long names).
- **--sidecar**: Sidecar type (e.g. `docker`).
