...
- **Any server** can serve the API NodePort — clients can hit any of `node1..3` on `:31007` over HTTPS (one cluster CA, `/etc/k7/tls/ca.crt` on the first master, covers every node IP). If you installed with `--api-allow-cidr`, the Service switches to `externalTrafficPolicy: Local`, so only the node actually running the `k7-api` pod answers — point clients at that node (or run a load balancer in front that health-checks `:31007`).
- **etcd quorum** spans the three servers. One server can fail without losing the control plane.
- **Backend labels** decide where a sandbox lands. `--backend kfd` is restricted to `node1` and `node4`; `--backend kql` to `node1`, `node2`, `node3`; `--backend k7d` (and `k7d-fc`, which shares the daemon and adds the `k7-fc` RuntimeClass) to `node1` and `node2`.
- **k7d nodes** run the node-local `k7d` daemon (systemd unit, owns every `runtimeClassName: k7` and `k7-fc` microVM on that node) plus its XFS disk pool at `/var/lib/k7d/disks`. `k7d-fc` nodes additionally carry the pinned Firecracker + jailer binaries (`/usr/local/bin/firecracker`, `jailer`) and `/etc/k7d/shim-k7-fc.toml`.
...
```ini
[k7_servers]
node1 ansible_host=203.0.113.10 k7_backends=kata-qemu-longhorn,kata-firecracker-devmapper,k7d k7_devmapper_disk=/dev/nvme1n1
node2 ansible_host=203.0.113.11 k7_backends=kata-qemu-longhorn,k7d
...
For a 3-node HA cluster with **all backends on every node** (`kfd`, `kql`, `k7d`, `k7d-fc`), one command from the first master brings everything up. Clone [Katakate/k7](https://github.com/Katakate/k7) at `v0.4.0` on that master and run `k7 install` from the checkout (`k7-api:local` is built from cwd). Fresh nodes have no SSH host keys yet — either `ssh-keyscan` the peers into `known_hosts` or set `ansible_ssh_common_args` as below:
...
```bash
k7 install -i inventory.ini --ha --k7d-artifact /root/k7d-v0.7.0-x86_64-linux.tar.gz
```

`--k7d-version 0.7.0` downloads the public GitHub release instead (also the playbook default). A tarball you built from k7d source is `--k7d-artifact`. k7 **0.4.0** copies Firecracker **v1.16.2** for `k7d-fc`.

Two-node (one server + one agent, **no `--ha`** — embedded etcd needs 3+ servers). After each box has Ubuntu on one NVMe and a raw spare (`TWO_DISK`):
...
| `k7_backends` | yes | Comma-separated list of backends supported by this node. Values: `kata-qemu-longhorn` (`kql`), `kata-firecracker-devmapper` (`kfd`), `k7d`, `k7d-fc` (`k7-fc`) |
| `k7_devmapper_disk` | for `kata-firecracker-devmapper` | **Raw, unformatted** block device for the LVM thin-pool (e.g. `/dev/nvme1n1`). Usually better left unset — the playbook auto-detects (see note above). Wipe with `utils/wipe-disk.sh` first. On dual-NVMe (`TWO_DISK`) boxes **omit this** so a swapped `nvme0n1`/`nvme1n1` cannot point at the OS disk. |
