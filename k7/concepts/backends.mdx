...
**Privilege separation** is the k7d daemon, not a fifth backend: a root node broker (no listening socket) plus a uid-`k7d` VM broker for the whole node. Both `k7d` and `k7d-fc` sandboxes go through that pair. Tables: [k7d vs k7d-fc](/k7d/concepts/backends#k7d-vs-k7d-fc) and [privilege separation](/k7d/concepts/security#privilege-separation).
...
```bash
k7 install --backend kfd                       # required --backend; kata-firecracker-devmapper only (auto-detects raw disk)
...
k7 install --backend k7d --k7d-version 0.7.0   # pin the k7d release (playbook default: 0.7.0)
k7 install --backend k7d --k7d-artifact /root/k7d-v0.7.0-x86_64-linux.tar.gz   # install from a local tarball
...
```bash
# Auto-detect (uses /etc/k7/backend on the API node, else kfd)
k7 create demo alpine:3.20 --cpu 1 --memory 1Gi

# Explicit
k7 create demo alpine:3.20 --backend kql --cpu 1 --memory 1Gi
k7 create demo alpine:3.20 --backend k7d --cpu 1 --memory 1Gi
k7 create builder ubuntu:24.04 --backend k7d-fc --docker --egress-open --cpu 2 --memory 3Gi
```
