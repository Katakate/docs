...
<Note>
These pages track **k7 0.3.1** (HTTPS-by-default, `--docker`, `k7d-fc`, Cilium isolation; HA Firecracker-pin copy). The playbook pins [k7d 0.6.0](https://github.com/Katakate/k7d/releases/tag/v0.6.0). Older PPA packages (0.2.2 and earlier) still serve the API over plain HTTP and pin an older k7d. Read the [CHANGELOG](https://github.com/Katakate/k7/blob/main/CHANGELOG.md).
</Note>
...
The Launchpad PPA currently publishes **0.2.2**. These pages track **0.3.1** — install the [GitHub release `.deb`](https://github.com/Katakate/k7/releases/tag/v0.3.1):
...
When Launchpad catches up, `sudo add-apt-repository ppa:katakate.org/k7 && sudo apt update && sudo apt install k7` will install 0.3.1 too. Confirm with `k7 -V`.
...
`k7 install` with no flags provisions **both Kata backends** (`kfd` + `kql`) on localhost. Add `k7d` for the warm-fork microVM runtime and `k7d-fc` for jailed Firecracker (0.3.1 copies the Firecracker pins from the repo root). The playbook pins [k7d 0.6.0](https://github.com/Katakate/k7d/releases/tag/v0.6.0).
