...
| **Redis 7** (AOF on) with an `INCR` loop mid-write on another node | manifests | **575 ms** client / 568 ms daemon; the child continues from the in-flight write |
| Held apiserver `watch` + ClusterIP TCP stream through the fork | in-guest | streams survive on source and child, then diverge |
...
## Surviving a daemon crash

The on-disk tree outlives the daemon that forked it. If `k7d` is killed with `SIGKILL`, live KVM state dies with the process, but `{K7D_TREE_ROOT}/{tree_id}/tree.json` and every keyframe stay on disk: after `systemctl start k7d`, `tree_open` re-opens the tree with every node `Suspended`, `tree_resume root` brings the cluster back Ready from its last point, and `tree_fork_batch` forks it again — no source re-boot, no image pull. This is operational rehydrate, not live-KVM resurrection; a member's in-memory state since its last keyframe is lost.
