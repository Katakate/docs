...
4. On a cluster node only: `/etc/k7/api_endpoint`, `/etc/k7/tls/ca.crt`, and
   the first entry of `/etc/k7/api_keys.json`.
...
k7 install --backend kfd,kql,k7d,k7d-fc        # all four backends (k7d release pinned by the playbook)
...
k7 create --docker --backend k7d --docker-disk 40Gi builder ubuntu:24.04
...
k7 list [-n NAMESPACE]
```
Lists sandboxes with status, readiness, restarts, age, image, backend, node, and the NodePorts allocated by `--expose-port` (empty when the sandbox is not exposed).
