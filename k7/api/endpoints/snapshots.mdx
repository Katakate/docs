...
- `overrides` (optional) — any combination of `image`, `backend`,
  `root_disk_size`, `sidecar` (`"docker"` is the deprecated alias of
  `docker: true`; restore does not accept `docker` / `docker_disk` yet),
  `limits`, `entrypoint`, `cmd`, `before_script`. Each override wins over the snapshot's
