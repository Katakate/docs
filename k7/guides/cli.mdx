...
k7 install --backend kfd,kql,k7d,k7d-fc       # Kata + k7d + jailed Firecracker
k7 install --backend k7d --k7d-artifact /root/k7d-v0.6.0-x86_64-linux.tar.gz
...
- **-v**: verbose output
- **--backend, -b**: comma-separated backends: `kata-firecracker-devmapper` (`kfd`), `kata-qemu-longhorn` (`kql`), `k7d`, `k7d-fc`. Default installs the two Kata backends; add `k7d` for the warm-fork microVM runtime and `k7d-fc` for the same daemon driving jailed Firecracker (installs pinned Firecracker v1.16.1 + jailer, RuntimeClass `k7-fc`).
...
- **--k7d-version**: k7d release to download (k7d backends only). Playbook default is **`0.6.0`** (Firecracker engine + guest docker payload). Override with `--k7d-artifact` for a local tarball.
...
k7 create --name open --image alpine:latest --egress-open
...
k7 api endpoint      # just the URL (machine-readable, single line) — https://<first-master>:31007
