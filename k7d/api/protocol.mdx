...
| `vcpus` | u32 | `1` | Guest vCPU count. **The VMM is single-vCPU today** — values > 1 are clamped to 1 with a loud warning (multi-vCPU is in progress); the CFS quota below still enforces the pod's CPU limit host-side |
